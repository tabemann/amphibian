          ...
          case T.uncons firstPart of
            '/' : firstPartRest ->
              case T.uncons firstPartRest of
                ...
                _ ->
                  let (command, commandRest) = T.breakOn (T.singleton ' ') firstPartRest in
                  let (_, commandRest) = T.span isSpace commandRest in
                  dispatchCommand dispatcher (idfmFrame mapping) command (StyledTextElement style commandRest : rest)
